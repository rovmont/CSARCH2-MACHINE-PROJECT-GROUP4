...
export function roundDigitString(digits, targetDigits, mode, sign = 0, radix = 'decimal') {
  ...
  if (safeTarget >= clean.length) {
    steps.push(`Digit string "${clean}" already fits within ${safeTarget} digit(s); no rounding needed.`);
    return { digits: clean, steps, changed: false };
  }

  const kept = clean.slice(0, safeTarget) || '0';
  const discarded = clean.slice(safeTarget);
  steps.push(`Keeping first ${safeTarget} digit(s): "${kept}", discarding "${discarded}".`);
  ...
  switch (mode) {
    case 'chopping': {
      steps.push('Chopping (truncation toward 0): drop the discarded digits, never increment.');
      break;
    }
    case 'round-up': { //toward +∞
      if (allZero) {
        steps.push('Discarded portion is all zeros — value is exact, no change needed.');
      } else if (sign === 0) {
        steps.push('Rounding toward +∞ on a positive number with nonzero discarded digits: increment the kept digits.');
        roundUpMagnitude = true;
      } else {
        steps.push('Rounding toward +∞ on a negative number: moving toward +∞ shrinks the magnitude, so just truncate.');
      }
      break;
    }
    case 'round-down': { //toward −∞
      if (allZero) {
        ...
        steps.push('Rounding toward −∞ on a negative number with nonzero discarded digits: increment the magnitude.');
        roundUpMagnitude = true;
      } else {
        steps.push('Rounding toward −∞ on a positive number: moving toward −∞ shrinks the magnitude, so just truncate.');
      }
      break;
    }
    case 'ties-to-even': {
      ...
      const half = String(base / 2) + '0'.repeat(discarded.length - 1);
      if (discarded > half) {
        steps.push(`Discarded "${discarded}" > halfway point "${half}": round up.`);
        roundUpMagnitude = true;
      } else if (discarded < half) {
        steps.push(`Discarded "${discarded}" < halfway point "${half}": round down.`);
      } else {
        const lastKeptDigit = Number(kept[kept.length - 1] ?? '0');
        const isOdd = lastKeptDigit % 2 !== 0;
        roundUpMagnitude = isOdd;
        steps.push(
          `Discarded "${discarded}" is exactly halfway. Ties-to-even: last kept digit is ${lastKeptDigit} ` +
          `(${isOdd ? 'odd' : 'even'}), so ${isOdd ? 'round up to make it even' : 'keep it, already even'}.`
        );
      }
      break;
    }
  ...
  if (roundUpMagnitude) {
    ...
    steps.push(`Final rounded digits: "${resultDigits}".`);
  }

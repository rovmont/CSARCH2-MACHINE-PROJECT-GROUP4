...
/**
 * Bit-level decimal32 DPD codec (pack / unpack).
 *
 * Layout: [sign 1] [combination 5] [exp continuation 6] [declet0 10] [declet1 10]
 */
...
export function encodeDecimal32(value) {
  ...
  return (
    sign +
    combination +
    expCont6.toString(2).padStart(6, '0') +
    declet1.toString(2).padStart(10, '0') +
    declet2.toString(2).padStart(10, '0')
  );
}
...
export function decodeDecimal32(bitsOrHex) {
  ...
  const coefficient =
    String(msd) + String(d1) + String(d2) + String(d3) + String(d4) + String(d5) + String(d6);
